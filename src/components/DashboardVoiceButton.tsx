...
export function DashboardVoiceButton({
      ...
      recorder.onstop = () => {
        ...
        if (shouldSubmitRef.current && chunksRef.current.length > 0) {
          ...
          voiceApi
            ...
            .catch((err) => {
              const msg = err?.response?.data?.error || err?.message || 'Voice processing failed';
              onError?.(msg);
            })
            .finally(() => {
              setProcessing(false);
  ...
  }, [onResult, onError, outletId, stopStream, updateLevels]);
  ...
  if (processing) {
    return (
      <div
        className={`flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-200 ${className}`}
      >
  ...
  if (recording) {
    return (
      <div
        className={`flex items-center gap-3 px-4 py-2.5 rounded-xl bg-red-50 border border-red-200 ${className}`}
      >
