...
test.describe('Tasks', () => {
  test('one-time task requires date before save', async ({ page }) => {
    ...
    await page.getByRole('option', { name: /one time only/i }).click();

    await page.locator('form').getByRole('button', { name: 'Create task', exact: true }).click();
    await expect(
      page.getByText('Date is required for one-time tasks', { exact: true }),
    ).toBeVisible();
  });
});
